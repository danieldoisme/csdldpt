import os
import argparse
from src.preprocess import preprocess_all_images
...
def main():
   ...
   parser = argparse.ArgumentParser(description='Hệ thống tìm kiếm ảnh lá cây tương tự')
   parser.add_argument('--mode', type=str, default='search', choices=['preprocess', 'train', 'search', 'all'],
                       help='Chế độ chạy: preprocess (tiền xử lý), train (xây dựng CSDL), search (tìm kiếm), all (tất cả)')
